        ...
        <div className="space-y-2">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
            <div className="flex items-center gap-2 text-foreground">
              {/* The pulse is the only signal that this session is live, and motion
                  carries nothing to a screen reader. */}
              <div className="h-3 w-3 animate-pulse rounded-full bg-green-600 dark:bg-green-500 flex-shrink-0" />
            ...
            <div className="text-center">
              ...
              <LiveTimer
                ...
                frozenAt={activeBreak?.startTime ?? null}
              />
          ...
          {!activeBreak && (
            <>
              ...
              {/* Clock Out leads on mobile and is the only filled button in the row.
                  It used to be `destructive` red and fifth in a stack of five
                  identical full-width blocks — the separation that sets it apart on
                  desktop (sm:ml-auto) does not apply below 640px. Ending a session
                  is a routine daily action; red is reserved for deleting a record.
                  The rest follow in frequency order. */}
              <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
                <Button
                  onClick={onClockOut}
                  variant="default"
                  className="gap-2 w-full sm:w-auto order-first sm:order-last sm:ml-auto min-h-[44px]"
                >
                  <Square className="h-4 w-4" />
                  Clock Out
                </Button>
                <Button onClick={onSwitchTask} variant="outline" className="gap-2 w-full sm:w-auto min-h-[44px]">
